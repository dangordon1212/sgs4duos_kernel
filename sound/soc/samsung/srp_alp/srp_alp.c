...
#if 1
#define srp_enable(stat, func)
#else
static DEFINE_SPINLOCK(lock_srp_cnt);

static int srp_cnt;

static void srp_enable(int stat, const char *func)
{
	unsigned long flags;

	spin_lock_irqsave(&lock_srp_cnt, flags);
	...
	srp_info("%s: srp_cnt %d(%s)\n", __func__, srp_cnt, func);

	spin_unlock_irqrestore(&lock_srp_cnt, flags);
}
#endif
